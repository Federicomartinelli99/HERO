...
logger = setup_logger("reconcile_pipeline_v5_final")
...
def main():
    ...
    wfp_raw_path = raw_dir / "wfp.parquet"  # Loaded from raw/wfp.parquet containing 831 columns
    wfp_pcodes_path = data_dir / "wfp_with_pcodes.parquet" # Contains the PIP mapping results
    
    logger.info("Verifica dell'esistenza dei file raw...")
    for p in [ipc_raw_path, acled_raw_path, idp_raw_path, rain_raw_path, wfp_raw_path, wfp_pcodes_path]:
        if not p.exists():
    ...
    # Estraiamo colonne pass-through
    ...
    logger.info("Avvio del modulo di riconciliazione WFP Prices per v5 con TUTTE le commodity...")
    
    logger.info("Caricamento wfp.parquet raw...")
    df_wfp_raw = pd.read_parquet(wfp_raw_path)
    logger.info("Caricamento wfp_with_pcodes.parquet per recuperare i PCodes spaziali...")
    df_wfp_pc = pd.read_parquet(wfp_pcodes_path)
    
    # Per sicurezza uniamo i PCode basati su chiavi univoche
    logger.info("Arricchimento wfp_raw con PCode mappati geograficamente...")
    merge_keys = ['ISO3', 'mkt_name', 'lat', 'lon', 'year', 'month']
    pcode_cols = ['adm1_pcode', 'adm2_pcode', 'mapping_method_adm1', 'mapping_method_adm2']
    
    df_wfp_raw = df_wfp_raw.drop(columns=pcode_cols, errors='ignore')
    df_wfp_src = df_wfp_raw.merge(
        df_wfp_pc[merge_keys + pcode_cols].drop_duplicates(subset=merge_keys),
        on=merge_keys,
        how='left'
    )
    logger.info(f"Shape wfp arricchito con PCode: {df_wfp_src.shape}")
    ...
    # Identifichiamo tutte le colonne commodity da aggregare
    exclude_prefixes = ["DATES", "currency", "components", "start_dense_data", "last_survey_point", "geo_id"]
    agg_cols = [
        col for col in df_wfp_src.columns
        if any(col.startswith(pfx) for pfx in ["c_", "inflation_", "o_", "h_", "l_", "trust_"])
        and col not in exclude_prefixes
    ]
    logger.info(f"Trovate {len(agg_cols)} colonne commodity da aggregare e riconciliare.")
    
    logger.info("Pre-aggregazione dei vari livelli di WFP...")
    wfp_levels = {
        "admin2": (
            df_wfp_src[df_wfp_src["adm2_pcode"].notna() & (df_wfp_src["adm2_pcode"] != "")]
            .groupby(["ISO3", "adm2_pcode", "date"], observed=True)
            .agg(**{c: (c, "mean") for c in agg_cols},
                 mapping_method=(f"mapping_method_adm2", lambda x: "elastic_buffer" if "elastic_buffer" in x.values else "strict_pip"))
            .reset_index()
        ),
        "admin1_code": (
            ...
            .agg(**{c: (c, "mean") for c in agg_cols},
                 mapping_method=(f"mapping_method_adm1", lambda x: "elastic_buffer" if "elastic_buffer" in x.values else "strict_pip"))
            .reset_index()
        ),
        "admin1_name": (
    ...
    logger.info("Avvio del merge gerarchico per tutte le commodity...")
    wfp_chunks = []
    wfp_matched_ids = set()
    ...
    for label, key, left_keys, right_keys, filter_col in level_specs:
        ...
        logger.info(f"  Elaborazione livello {label} ({len(remaining)} righe rimanenti)...")
        
        # 1. Esplodiamo remaining per mesi in modo da evitare la OOM error
        remaining_exploded = remaining.copy()
        remaining_exploded["date"] = remaining_exploded.apply(
            lambda r: pd.date_range(start=r["date_from"], end=r["date_to"], freq="MS"),
            axis=1
        )
        remaining_exploded = remaining_exploded.explode("date").dropna(subset=["date"])
        remaining_exploded["date"] = pd.to_datetime(remaining_exploded["date"])
        
        # 2. Eseguiamo il merge temporale e spaziale preciso (no cartesian expansion!)
        m = pd.merge(
            remaining_exploded[["ipc_row_id", "date"] + left_keys],
            wfp_levels[key],
            left_on=left_keys + ["date"],
            right_on=right_keys + ["date"],
            how="inner"
        )
        ...
        # 3. Raggruppiamo di nuovo per ipc_row_id per calcolare la media delle commodity
        agg_funcs = {col: (col, "mean") for col in agg_cols}
        agg_funcs["wfp_obs_count"] = ("c_food_price_index", "size")
        agg_funcs["wfp_spatial_mapping_method"] = (
            "mapping_method", 
            lambda x: "elastic_buffer" if "elastic_buffer" in x.values else ("national_fallback" if "national_fallback" in x.values else "strict_pip")
        )
        ...
        rename_dict = {col: f"WFP_avg_{col}" for col in agg_cols}
    ...
    if wfp_chunks:
        ...
        out_cols = ["ipc_row_id", "wfp_match_level", "wfp_obs_count", "wfp_spatial_mapping_method"] + [f"WFP_avg_{c}" for c in agg_cols]
        wfp_final = pd.DataFrame(columns=out_cols)
    ...
    if "WFP_avg_c_food_price_index" in df_consolidated.columns:
        df_consolidated["WFP_avg_price"] = df_consolidated["WFP_avg_c_food_price_index"]
    if "WFP_avg_inflation_food_price_index" in df_consolidated.columns:
        df_consolidated["WFP_avg_inflation"] = df_consolidated["WFP_avg_inflation_food_price_index"]
    ...
    df_consolidated["has_wfp"] = df_consolidated["WFP_avg_price"].notna() if "WFP_avg_price" in df_consolidated.columns else False
    ...
    root_reconciled = workspace_dir / "hero_v5_reconciled_v5.parquet"
    df_consolidated.to_parquet(root_reconciled, index=False, engine=PARQUET_ENGINE)
    logger.info(f"Copia salvata anche nel root del workspace: {root_reconciled}")
    
    elapsed = time.time() - t0
    logger.info("==================================================")
    logger.info(f"[OK] PIPELINE ENRICHED DI TUTTE LE COMMODITY COMPLETATA CON SUCCESSO in {elapsed:.2f}s!")
