...
Each market is a `(lat, lon)` point. WFP arrives **pre-mapped** in
`data/raw/wfp_with_pcodes.parquet`: an upstream PIP step ran *twice* per market — once
against adm1 polygons, once against adm2 polygons — and wrote both `adm1_pcode` and
`adm2_pcode`, plus a `mapping_method_adm{1,2}` flag
...
IPC row at adm1: ("ETH", "ET01", admin_level=1, 2023-04 → 2023-09)
   ...
   events in    snapshot of mm of   prices     merged_adm1
   ET01 in      ≤ ipc_end ET01-     reported   .parquet
   that period            level     in ET01    (long form)
                          rain      markets
                          rows in
                          period
```
...
they are not the same quantity. Coverage percentages (share of rows with each theme
attached) are comparable because they're relative; absolute values are not.
