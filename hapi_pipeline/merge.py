...
Produces two outputs, one per admin level, driven by IPC's own `admin_level` column:
  - data/final/hapi_merged_2017_adm1.parquet  — only IPC rows reported at admin1
  - data/final/hapi_merged_2017_adm2.parquet  — only IPC rows reported at admin2
...
Aggregation rules:
  ...
  - WFP:      per-period mean price + mean inflation + observation count, with worst-case
              ...
              it, else 'strict_pip'). WFP source is hero_v5/data/wfp_with_pcodes.parquet,
              produced by hero_v5/libs/wfp_consolidate.py + wfp_spatial_mapping.py.
...
def aggregate_wfp(wfp, join_key_col, level, ipc_periods):
    ...
    Input schema (from hero_v5/data/wfp_with_pcodes.parquet):
...
def main():
    ...
    if wfp_all.empty:
        ...
        print(f"       Run hero_v5/libs/wfp_consolidate.py then wfp_spatial_mapping.py to produce it.")
